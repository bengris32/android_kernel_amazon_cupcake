...
static int tmp103_get_temp(struct thermal_dev *tdev, int *temp)
{
	...
	if (unlikely(current_temp >= TMP103_MAX_TEMP)) {
		if (++fail_count > MAX_FAULTY_RETURN_COUNT) {
			...
			*temp = tmp103->therm_fw->current_temp;
			return 0;
		}
...
static int tmp103_sensor_get_temp(void *data, int *t)
{
	struct thermal_dev *therm_fw = data;

	return tmp103_get_temp(therm_fw, t);
}

static struct thermal_zone_of_device_ops tmp103_sensor_ops = {
	.get_temp = tmp103_sensor_get_temp,
};
...
static int tmp103_temp_sensor_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (IS_ERR(thermal_zone_of_sensor_register(tmp103->dev,
			  0,  tmp103->therm_fw, &tmp103_sensor_ops)))
