...
static uint16_t cm3232_white_adc;
...
static void read_ls_adc_value(void)
{
  ...
  mutex_lock(&als_get_adc_mutex);
	...
	_cm3232_I2C_Read_Word(lpi->slave_addr, CM3232_WHITE_DATA, &cm3232_white_adc);
...
static void report_lsensor_input_event(struct cm3232_info *lpi)
{
  ...
  D("[LS][CM3232] %s [ADC:%04x]: %d lux \n", __func__, lpi->current_adc, lpi->current_lux);
	...
	lpi->white_lux = (uint32_t)div64_u64((uint64_t)cm3232_white_adc * lpi->cal_data, (uint64_t) 100000);
  lpi->white_adc = cm3232_white_adc;
  D("[LS][CM3232] %s [WHITE ADC:%04x]: %d white lux \n", __func__, lpi->white_adc, lpi->white_lux);
...
static ssize_t ls_white_channel_adc_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	struct cm3232_info *lpi = lp_info;
  return sprintf(buf, "[LS] ADC[0x%04X] => %d lux\n",
		lpi->white_adc, lpi->white_lux);
}
...
static struct device_attribute dev_attr_ls_white_channel_adc =
__ATTR(ls_white_channel_adc, 0664, ls_white_channel_adc_show, NULL);
...
static struct attribute *light_sysfs_attrs[] = {
	...
	&dev_attr_ls_white_channel_adc.attr,
