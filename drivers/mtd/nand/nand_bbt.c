...
{
	uint8_t msk = BBT_ENTRY_MASK << ((block & BBT_ENTRY_MASK) * 2);
	chip->bbt[block >> BBT_ENTRY_SHIFT] &= ~msk;
}
...
/**
 * nand_isworn_bbt - [NAND Interface] Check if a block is worn bad
 * @mtd: MTD device structure
 * @offs: offset in the device
 */
int nand_isworn_bbt(struct mtd_info *mtd, loff_t offs)
{
	struct nand_chip *this = mtd->priv;
	int block, res;

	block = (int)(offs >> this->bbt_erase_shift);
	res = bbt_get_entry(mtd->priv, block);

	return (res == BBT_BLOCK_WORN) ? 1 : 0;
}
