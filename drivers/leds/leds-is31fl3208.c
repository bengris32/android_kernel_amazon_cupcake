...
#define REG_CTRL_BASE 0x14
#define REG_G_CTRL 0x26 /* Global Control Register */
#define REG_RST 0x2F
...
#define LED_CURRENT_0   0x00
#define LED_CURRENT_1   0x10
#define LED_CURRENT_1_2 0x11
#define LED_CURRENT_1_3 0x12
#define LED_CURRENT_1_4 0x13
#define LED_CURRENT_DEFAULT LED_CURRENT_1_4

#define BOOT_ANIMATION_FRAME_DELAY 88

struct is31fl3208_data {
	struct mutex lock;
...
static int is31fl3208_write_reg(struct i2c_client *client,
				uint32_t reg,
				uint8_t value)
{
...
static int is31fl3208_update_led(struct i2c_client *client,
				 uint32_t led,
				 uint8_t value)
{
	return is31fl3208_write_reg(client, REG_PWM_BASE+led, value);
}

static int update_frame(struct is31fl3208_data *pdata,
						const uint8_t *buf)
{
	...
	for (i = 0; i < NUM_CHANNELS; i++) {
		pdata->state[i] = buf[i];
		ret = is31fl3208_update_led(client,
				(i+pdata->ch_offset)%NUM_CHANNELS,
				pdata->state[i]);
		if (ret != 0)
			goto fail;
	}
	ret = is31fl3208_write_reg(client, REG_UPDATE, 0x0);
fail:
...
{
	struct is31fl3208_data *pdata = (struct is31fl3208_data *)data;
	int i = 0;
...
static ssize_t boot_animation_store(struct device *dev,
				    ...
				    size_t len)
{
	struct is31fl3208_data *pdata = dev_get_platdata(dev);
	uint8_t val;
	ssize_t ret;
				 ...
				 size_t len)
{
	...
	if (val < LED_CURRENT_0 || val > LED_CURRENT_1_4) {
	...
	val = (val << 1) | 0x1;
	for (i = 0; i < NUM_CHANNELS; i++)
		is31fl3208_write_reg(client, REG_CTRL_BASE + i, val);

	is31fl3208_write_reg(client, REG_UPDATE, 0x0);
	mutex_unlock(&pdata->lock);
	ret = len;
fail:
	return ret;
...
static void is31fl3208_parse_dt(struct is31fl3208_data *pdata,
...
static int is31fl3208_power_on_device(struct i2c_client *client)
{
	struct is31fl3208_data *pdata = dev_get_platdata(&client->dev);
	int i;
	int ret = 0;
	...
	if (pdata->setup_device) {
		...
		ret = is31fl3208_write_reg(client, REG_RST, 0x0);
		if (ret < 0) {
			pr_err("ISSI: Could not reset registers\n");
			goto fail;
		}

		ret = is31fl3208_write_reg(client, REG_SW_SHUTDOWN, LED_SW_ON);
		if (ret < 0) {
	...
	for (i = 0; i < NUM_CHANNELS; i++) {
		ret = is31fl3208_write_reg(client, REG_CTRL_BASE + i, LED_CURRENT_DEFAULT);
		if (ret < 0) {
...
static int is31fl3208_reboot_callback(struct notifier_block *self,
				      unsigned long val,
				      void *data)
{
	int ret;

	struct is31fl3208_data *pdata =
		container_of(self, struct is31fl3208_data, reboot_notifier);
	pr_err("ISSI: resetting device before reboot!\n");
	ret = is31fl3208_write_reg(pdata->client, REG_RST, 0x0);
	if (ret < 0) {
		pr_err("ISSI: Could not reset registers\n");
...
static int is31fl3208_probe(struct i2c_client *client,
			    const struct i2c_device_id *id)
{
	struct is31fl3208_data *pdata = devm_kzalloc(&client->dev,
						sizeof(struct is31fl3208_data),
						GFP_KERNEL);
	int ret = 0;
	...
	pdata->boot_anim_task = NULL;
	pdata->reboot_notifier.notifier_call = is31fl3208_reboot_callback;
	is31fl3208_parse_dt(pdata, client->dev.of_node);

	client->dev.platform_data = pdata;
	ret = is31fl3208_power_on_device(client);
	if (ret < 0) {
...
static int is31fl3208_remove(struct i2c_client *client)
{
	struct is31fl3208_data *pdata = dev_get_platdata(&client->dev);
	int ret = 0;
...
static struct i2c_device_id is31fl3208_i2c_match[] = {
	{"issi,is31fl3208", 0},
};
MODULE_DEVICE_TABLE(i2c, is31fl3208_i2c_match);

static struct of_device_id is31fl3208_of_match[] = {
	{ .compatible = "issi,is31fl3208"},
};
MODULE_DEVICE_TABLE(of, is31fl3208_of_match);

static struct i2c_driver is31fl3208_driver = {
	.driver = {
		.name = "is31fl3208",
		.of_match_table = of_match_ptr(is31fl3208_of_match),
	},
	.probe = is31fl3208_probe,
	.remove = is31fl3208_remove,
	.id_table = is31fl3208_i2c_match,
};

module_i2c_driver(is31fl3208_driver);

MODULE_AUTHOR("Amazon.com");
MODULE_DESCRIPTION("ISSI is31fl3208 LED Driver");
MODULE_LICENSE("GPL");
