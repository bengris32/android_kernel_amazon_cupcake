...
#define IDME_OF_ALSPARAMS	"/idme/alsparams"
...
/*
 * The format of the idme alsparams field is "key1=value1 key2=value2 ..."
 */
int idme_get_alsparams_value(const char *key, unsigned int *value)
{
	struct device_node *ap = NULL;
	...
	ap = of_find_node_by_path(IDME_OF_ALSPARAMS);
	if (ap) {
		alscal_idme = (char *)of_get_property(ap, "value", NULL);
	} else {
		pr_debug("ALSCAL: could not get alsparams idme entry\n");
		return -EINVAL;
	}

	/* Copy string to a writable memory for strsep() */
	strncpy(alscal_str, alscal_idme, 64);
	ptr = strstr(alscal_str, key);

	if (NULL == ptr) {
		pr_err("ALSCAL: key (%s) does not exist\n", key);
		return -EINVAL;
	}
	...
	if (NULL == ptr) {
		pr_err("ALSCAL: alsparams format incorrect\n");
		return -EINVAL;
	}

	if (kstrtouint(ptr+1, 0, &res)) {
		pr_err("ALSCAL: unable to parse value for key: %s\n", key);
		return -EINVAL;
	}
...
EXPORT_SYMBOL(idme_get_alsparams_value);
