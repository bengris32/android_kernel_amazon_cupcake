...
#define AUXADC_CHANNEL_AUX_IN0 0
#define AUXADC_CHANNEL_AUX_IN1 1
#define AUXADC_CHANNEL_AUX_IN2 9
#define AUXADC_CHANNEL_AUX_IN3 12
#define AUXADC_CHANNEL_AUX_IN4 13
#define AUXADC_CHANNEL_AUX_IN5 14

#define AUXADC_MODE_IMM 0  /* immediate mode */
#define AUXADC_MODE_AUTO 1  /* auto-set mode */
#define AUXADC_MODE_RES 2  /* internal reserve mode */

static const int auxadc_ext_channels[] = {
	AUXADC_CHANNEL_AUX_IN0, AUXADC_CHANNEL_AUX_IN1, AUXADC_CHANNEL_AUX_IN2,
	AUXADC_CHANNEL_AUX_IN3, AUXADC_CHANNEL_AUX_IN4, AUXADC_CHANNEL_AUX_IN5
};

/* ADC channels reserved for internal use */
static const int auxadc_is_channel_reserved[ADC_CHANNEL_MAX] = {
	[PAD_AUX_THE0] = 1, [PAD_AUX_THE1] = 1
};
...
static int mt_auxadc_get_mode(int dwChannel)
{
	/* Verify whether the ADC channel is reserved for internal use */
	if (auxadc_is_channel_reserved[dwChannel]) {
		pr_debug("[AUXADC]: channel %d is in internal reserve mode\n",
			dwChannel);
		return AUXADC_MODE_RES;
	}

	/* Verify whether the ADC channel is in auto set mode */
	if (AUXADC_DRV_ReadReg16((volatile u16 *)AUXADC_CON0) &
		(1 << dwChannel)) {
		pr_debug("[AUXADC]: channel %d is in auto-set mode\n",
			dwChannel);
		return AUXADC_MODE_AUTO;
	}

	pr_debug("[AUXADC]: channel %d is in immediate mode\n", dwChannel);
	return AUXADC_MODE_IMM;
}
...
static int IMM_auxadc_GetOneChannelValue(int dwChannel, int data[4], int *rawdata)
{
	...
	ret = enable_clock(MT_PDN_PERI_AUXADC, "AUXADC");
	if (ret) {
		pr_err("hwEnableClock AUXADC failed.");
		mutex_unlock(&mutex_get_cali_value);
		return ret;
...
#endif
#endif
	if (mt_auxadc_get_mode(dwChannel) != AUXADC_MODE_IMM) {
		mutex_unlock(&mutex_get_cali_value);
		pr_info("[adc_api]: channel %d not in immediate mode, not allowed for immediate read\n",
			dwChannel);
		return -EINVAL;
	}
...
#else
#ifndef CONFIG_MTK_FPGA
	ret = disable_clock(MT_PDN_PERI_AUXADC, "AUXADC");
	if (ret) {
		pr_err("hwdisableClock AUXADC failed.");
		mutex_unlock(&mutex_get_cali_value);
		return ret;
...
static long auxadc_cali_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SET_ADC_CALI_Slop:
		...
		if (copy_from_user(auxadc_cali_slop, nvram_data_addr, 36))
			goto err_copy;
		g_AUXADC_Cali = false;
		/* Protection */
		for (i = 0; i < ADC_CHANNEL_MAX; i++) {
	...
	case SET_ADC_CALI_Offset:
		nvram_data_addr = (long *)arg;
		if (copy_from_user(auxadc_cali_offset, nvram_data_addr, 36))
			goto err_copy;
		g_AUXADC_Cali = false;
		for (i = 0; i < ADC_CHANNEL_MAX; i++)
	...
	case SET_ADC_CALI_Cal:
		nvram_data_addr = (long *)arg;
		if (copy_from_user(auxadc_cali_cal, nvram_data_addr, 4))
			goto err_copy;
	...
	case ADC_CHANNEL_READ:
		...
		if (copy_from_user(auxadc_in_data, user_data_addr, 8))
			/* 2*int = 2*4 */
			goto err_copy;

		pr_debug("this api is removed !!\n");
		if (copy_to_user(user_data_addr, auxadc_out_data, 8))
			goto err_copy;
...
static ssize_t show_AUXADC_chanel(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	/*
	 * Read only the external AUXADC channels. We don't read internal
	 * channels because reading thermal AUXADC channels 10 and 11 left
	 * them in a state after which thermal driver was unable to read good
	 * values for the CPU temp on AUXADC channels 10 and 11. If you are
	 * modifying this code to read all channels again, that should be done
	 * only after the issue with channel 10 and channel 11 is fixed.
	 */
	for (i = 0; i < ARRAY_SIZE(auxadc_ext_channels); i++) {
		int channel_id = auxadc_ext_channels[i];

		res = IMM_auxadc_GetOneChannelValue(channel_id, data, NULL);
		if (res < 0) {
			pr_debug("[adc_driver]: get data error\n");
		} else {
			pr_debug("[adc_driver]: channel[%d]=%d.%d\n",
						channel_id, data[0], data[1]);
