...
static int mtk_uart_startup(struct uart_port *port)
{
	...
	pinctrl_pm_select_default_state(uart->port.dev);
	return 0;
}
...
static void mtk_uart_shutdown(struct uart_port *port)
{
	...
	pinctrl_pm_select_sleep_state(uart->port.dev);
}
...
#else /* !defined(CONFIG_FPGA_EARLY_PORTING) */
	pr_debug("[UART][PinC]mtk_uart_probe CONFIG_FPGA_EARLY_PORTING is defined!\n");
#endif /* !defined(CONFIG_FPGA_EARLY_PORTING) */
