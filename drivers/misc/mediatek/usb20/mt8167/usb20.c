...
#define USB20_DEF_MODE  MUSB_PERIPHERAL

/* estimated debounce value based on experience */
#define VBUS_DEBOUN_INIT_MS  500
#define VBUS_DEBOUN_CONN_MS  3000
#define VBUS_DEBOUN_DISCONN_MS  500
#define VBUS_WAIT_MS  5000
...
void do_connection_work(struct work_struct *data)
{
	...
	if (!mtk_musb->power && (usb_in == true)) {
		...
		musb_stop(mtk_musb);
		if (wake_lock_active(&mtk_musb->usb_lock)) {
			/* keep holding wake lock for echo devices
			 * to avoid Android auto suspend
			 */
			/*DBG(0, "unlock\n");*/
			/*wake_unlock(&mtk_musb->usb_lock);*/
			DBG(0, "keep holding usb wake lock\n");
		} else {
...
static bool gpio_get_vbus_status(struct usb20_priv *data)
{
	bool vbus_exist = true;

	vbus_exist = !!gpio_get_value(data->usb_vbus_detect_pin);
	DBG(0, "vbus_exist:%s\n", vbus_exist ? "true" : "false");

	return vbus_exist;
}

static bool musb_hal_is_vbus_exist(struct usb20_priv *data)
{
	bool vbus_exist;

#ifdef BYPASS_PMIC_LINKAGE
	/*
	 * Only return false when we support vbus detect and dectect no
	 * external vbus existed
	 */
	if (data->vbus_detect_supported)
		vbus_exist = gpio_get_vbus_status(data);
	else
		vbus_exist = true;
#else
#if 1
...
void mt_usb20_sw_disconnect(void)
{
	if (!mtk_musb) {
		DBG(0, "mtk_musb = NULL\n");
		return;
...
bool usb_cable_connected(void)
{
	...
	if (musb_force_on) {
		...
		vbus_exist = musb_hal_is_vbus_exist(data);
	...
	if (is_usb_rdy() == false && mtk_musb->is_ready) {
						   ...
						   const char *buf, size_t count)
{
	...
	if (count >= 4 && !strncmp(buf, "idle", 4)) {
		if (device_mode) {
			...
			DBG(0, "do nothing\n");
		#ifdef CONFIG_MUSB_SUPPORT_Y_CABLE
		mtk_musb->force_mode = USB_FORCE_IDLE;
		#endif
	}
...
{
	DBG(0, "[MUSB]usb_y_cable_setup [%s]!\n", str);
	...
	DBG(0, "[MUSB]usb_y_cable value %d!\n", lk_y_cable);
...
static void musb_vbus_detect_work(struct work_struct *work)
{
	struct device *dev = mtk_musb->controller;
	struct musb_hdrc_platform_data *plat = dev_get_platdata(dev);
	struct usb20_priv *data = plat->board_data;
	int val = 0;

	val = gpio_get_value(data->usb_vbus_detect_pin);
	if (val) {
		DBG(0, "isr has happened since vbus detect pin value: %i, HIGH\n", val);

		mt_usb_connect();

		data->vbus_deboun_time = VBUS_DEBOUN_CONN_MS;
		irq_set_irq_type(data->usb_vbus_detect_irq, IRQF_TRIGGER_LOW);
	} else {
		DBG(0, "isr has happened since vbus detect pin: %i, LOW\n", val);
		data->vbus_deboun_time = VBUS_DEBOUN_DISCONN_MS;
...
{
	struct usb20_priv *data = dev_id;

	disable_irq_nosync(data->usb_vbus_detect_irq);
	if (!mtk_musb->is_ready)
		/* dealy 5 sec if usb function is not ready */
		schedule_delayed_work(&data->vd_pin_work, VBUS_WAIT_MS*HZ/1000);
	else
		schedule_delayed_work(&data->vd_pin_work,
						data->vbus_deboun_time*HZ/1000);

	DBG(0, "[musb] vbus pin interrupt assert\n");

	return IRQ_HANDLED;
}

static int mt_usb_ext_vbus_detect_init(struct musb *musb)
{
	...
	if (data->vbus_detect_supported) {
		data->pinctrl_vbus_detect = pinctrl_lookup_state(data->pinctrl, "vbus_detect_irq_init");
		if (IS_ERR(data->pinctrl_vbus_detect)) {
			ret = PTR_ERR(data->pinctrl_vbus_detect);
			DBG(0, "Cannot find usb pinctrl vbus_detect_irq_init (%d)\n", ret);
		} else {
			pinctrl_select_state(data->pinctrl, data->pinctrl_vbus_detect);
		}

		data->usb_vbus_detect_irq = gpio_to_irq(data->usb_vbus_detect_pin);
		DBG(0, "usb usb_iddig_number %d\n", data->usb_vbus_detect_irq);

		ret = request_irq(data->usb_vbus_detect_irq,
				mt_usb_vbus_detect_int, IRQF_TRIGGER_HIGH, "USB_VBUSD", data);
		if (ret)
			DBG(0, "USB VBUS DETECT IRQ LINE not available!!\n");
		else
			DBG(0, "USB VBUS DETECT IRQ LINE available!!\n");

		/* init vdpin interrupt if supported */
		INIT_DELAYED_WORK(&data->vd_pin_work, musb_vbus_detect_work);
	}
...
static int mt_usb_init(struct musb *musb)
{
	...
	INIT_WORK(&change_device_work, musb_change_device_work);
	setup_timer(&musb_device_timer, musb_change_device, (unsigned long) musb);
	#endif

	DBG(0, "%s, init_otg\n", __func__);
	ret = mt_usb_otg_init(musb);

	#ifdef CONFIG_MUSB_SUPPORT_Y_CABLE
	mod_timer(&musb_device_timer, jiffies + msecs_to_jiffies(6000));
	#endif

	ret = mt_usb_ext_vbus_detect_init(musb);
	return ret;
}
...
static int mt_usb_probe(struct platform_device *pdev)
{
	...
	if (mt_usb_otg_supported()) {
		...
		if (of_property_read_u32(np, "mode", (u32 *)&pdata->mode)) {
			pdata->mode = USB20_DEF_MODE;
			dev_warn(&pdev->dev, "set mode to default value %d\n", pdata->mode);
		}
	}
	...
	data->usb_vbus_detect_pin = of_get_named_gpio(np, "vbusdetect_gpio", 0);
	if (!gpio_is_valid(data->usb_vbus_detect_pin)) {
		dev_warn(&pdev->dev, "cannot find vbusdetect_gpio, vbus detection not supported\n");
		data->vbus_detect_supported = false;
	} else {
		DBG(0, "drvvbus_pin %d", data->usb_vbus_detect_pin);
		data->vbus_detect_supported = true;
		data->vbus_deboun_time = VBUS_DEBOUN_INIT_MS;
	}
	...
	ret = of_property_read_u8(np,
				   "eye-patten-vrt-vref-sel",
				   &config->vrt_vref);
	if (!ret)
		DBG(0, "Compliance param: VRT_VREF_SEL = 0x%08X\n", config->vrt_vref);
	else
		config->vrt_vref = EINVAL;

	ret = of_property_read_u8(np,
				   "eye-patten-term-vref-sel",
				   &config->term_vref);
	if (!ret)
		DBG(0, "Compliance param: TERM_VREF_SEL = 0x%08X\n", config->term_vref);
	else
		config->term_vref = EINVAL;

	ret = of_property_read_u8(np,
				   "eye-patten-hstx-srctrl",
				   &config->hstx_srctrl);
	if (!ret)
		DBG(0, "Compliance param: HSTX_SRCTRL = 0x%08X\n", config->hstx_srctrl);
	else
		config->hstx_srctrl = EINVAL;
