...
static void hs_slew_rate_cal(void)
{
	...
	struct musb_hdrc_platform_data *plat =
				dev_get_platdata(mtk_musb->controller);
	...
	if (plat->config->hstx_srctrl <= 7)
		value = plat->config->hstx_srctrl;
	else
		DBG(0,
		    "Invalid HSTX_SRCTRL (0x%02X). Use calculated value (%d)\n",
		    plat->config->hstx_srctrl, value);
...
void usb_phy_recover(void)
{
	...
	if (plat->config->vrt_vref <= 7) {
		USBPHY_CLR8(0x05, 0x70);
		USBPHY_SET8(0x05, (u8)(plat->config->vrt_vref << 4));
	} else {
		DBG(0,
		    "Invalid VRT_VREF_SEL (0x%02X), keep default value\n",
		    plat->config->vrt_vref);
	}

	if (plat->config->term_vref <= 7) {
		USBPHY_CLR8(0x05, 0x07);
		USBPHY_SET8(0x05, (u8)plat->config->term_vref);
	} else {
		DBG(0,
		    "Invalid TERM_VREF_SEL (0x%02X), keep default value\n",
		    plat->config->term_vref);
	}
