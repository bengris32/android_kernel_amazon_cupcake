...
struct notifier_block hps_rebooter;

static int hps_reboot_notifier(struct notifier_block *n, unsigned long val,
		void *v)
{
	hps_task_stop();
...
int hps_core_init(void)
{
	...
	if (r) {
		...
		hps_rebooter.priority = 1;
		hps_rebooter.notifier_call = hps_reboot_notifier;
		register_reboot_notifier(&hps_rebooter);
	}
...
int hps_core_deinit(void)
{
	...
	unregister_reboot_notifier(&hps_rebooter);
	hps_task_stop();
