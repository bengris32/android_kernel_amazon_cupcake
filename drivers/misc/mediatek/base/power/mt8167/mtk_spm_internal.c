...
void __spm_set_wakeup_event(const struct pwr_ctrl *pwrctrl)
{
	...
	if (mtk_pcm_timer_disable)
		val = 0;
	else
		val = CON1_PCM_TIMER_EN;
	spm_write(SPM_PCM_CON1, spm_read(SPM_PCM_CON1) | CON1_CFG_KEY | val);
...
int __spm_set_pcm_timer_state(bool state)
{
	mtk_pcm_timer_disable = !state;

	return 0;
}

int __spm_set_suspend_ctrl_timer(unsigned int val)
{
	__spm_suspend.pwrctrl->timer_val_cust = val;
...
static int __init spm_get_pwrap_base(void)
{
	...
	if (of_property_read_bool(node, "pcm-timer-disable")) {
		spm_info("pcm timer disabled\n");
		mtk_pcm_timer_disable = true;
	} else {
		spm_info("pcm timer enabled\n");
	}
