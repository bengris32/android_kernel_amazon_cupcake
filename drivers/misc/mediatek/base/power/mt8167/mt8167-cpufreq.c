...
static int mtk_cpufreq_init(struct cpufreq_policy *policy)
{
	...
	struct dev_pm_opp *opp;
	int opp_idx = 0;
	int	lv, project_code, function_code;
	...
	if (lv == 1) {
		...
		/* efuse = 0, default 1.3G or free = 1.6 */
		project_code = get_devinfo_with_index(45) & 0x3FFF;
		function_code = get_devinfo_with_index(48) & 0xF;
		/* case of no bonding */
		if (project_code == 0x2144 && function_code == 0x1) {
			/* add opp table by efuse */
			dev_pm_opp_add(info->cpu_dev, 1400000000, 1300000);
			dev_pm_opp_add(info->cpu_dev, 1500000000, 1300000);
			dev_pm_opp_add(info->cpu_dev, 1599000000, 1300000);
		}
	} else if (lv == 2) {
		/* efuse = 2, 1.3G */
	} else {
...
static const struct of_device_id mt8167_cpufreq_match[] = {
...
static struct platform_driver mt8167_cpufreq_platdrv = {
	.driver = {
		.name	= "mt8167-cpufreq",
		.of_match_table = of_match_ptr(mt8167_cpufreq_match),
	},
