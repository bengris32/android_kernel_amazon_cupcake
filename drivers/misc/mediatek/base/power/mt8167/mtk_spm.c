...
{
	return __spm_set_pcm_timer_state(state);
}
EXPORT_SYMBOL(spm_set_pcm_timer_state);

int spm_set_suspend_ctrl_timer(unsigned int time_s)
{
	/* timer val = seconds * 32768. This is done by << 15 */
	const unsigned int val = time_s << 15;

	return __spm_set_suspend_ctrl_timer(val);
}
EXPORT_SYMBOL(spm_set_suspend_ctrl_timer);
