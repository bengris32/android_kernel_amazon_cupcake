...
static ssize_t pcm_timer_show(struct kobject *kobj,
...
static ssize_t pcm_timer_store(struct kobject *kobj,
			struct kobj_attribute *attr,
			const char *buf, size_t n)
{
	char status[8];

	if (sscanf(buf, "%7s", status) != 1)
		return -EINVAL;

	spm_debug("pcm_timer_store: status = %s\n", status);

	if (!strcmp(status, "enable"))
		mtk_pcm_timer_disable = false;
