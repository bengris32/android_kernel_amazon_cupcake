...
static int __rt5738_set_mode(struct regulator_dev *rdev, unsigned int mode)
{
	struct rt5738_regulator_info *info = rdev_get_drvdata(rdev);
	struct rt5738_chip_data *chip = info->chip_data;
	int ret;
...
static int rt5738_set_mode(struct regulator_dev *rdev, unsigned int mode)
{
	...
	RT5738_INFO("%s: requested mode %d\n", __func__, mode);
	if (info->ignore_mode_change) {
		if (!info->initial_mode_flag) {
			RT5738_INFO("%s: setting initial mode, overriding ignore mode change\n",
					__func__);
			ret = __rt5738_set_mode(rdev, mode);
			info->initial_mode_flag = 1;
		} else {
			RT5738_INFO("%s: mode change ignored\n", __func__);
			ret = 0;
		}
	}
...
static unsigned int rt5738_of_map_mode(unsigned int mode)
{
	RT5738_INFO("%s: DT requested: %d\n", __func__, mode);

	switch(mode) {
		case RT5738_OPMODE_FAST:
			return REGULATOR_MODE_FAST;
		case RT5738_OPMODE_NORMAL:
		default:
			return REGULATOR_MODE_NORMAL;
	}
}
...
static struct regulator_desc rt5738_regulator_desc = {
	...
	.owner = THIS_MODULE,
	.of_map_mode = rt5738_of_map_mode,
};
			   ...
			   struct rt5738_regulator_info *info)
{
	...
	if (ret < 0)
		pr_warn("%s: no vsel_gpio info\n", __func__);
	info->vsel_gpio = ret;

	ret = of_property_read_u32(np, "ramp_up", &val);
	if (ret < 0) {
		pr_warn("%s: no ramp_up info, use default rate\n", __func__);
	...
	if (ret < 0) {
		pr_warn("%s: no ramp_down info, use default rate\n", __func__);
		info->ramp_down_val = 3;	/* 011 : 3mV/us */
	} else
		info->ramp_down_val = val;

	info->ignore_mode_change = of_property_read_bool(np, "ignore-mode-change");
								...
								*node_name)
{
	...
	init_data = of_get_regulator_init_data(dev, np, &rt5738_regulator_desc);
	if (init_data) {
		dev_info(dev,
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (init_data == NULL) {
		dev_err(&i2c->dev, "no init data\n");
		return -EINVAL;
	}

	info->initial_mode = init_data->constraints.initial_mode;
	info->i2c = i2c;
	info->dev = &i2c->dev;
	...
	if (IS_ERR(info->regulator)) {
		dev_err(&i2c->dev, "fail to register rt5738 regulator : %s\n",
	...
	if (ret < 0) {
		dev_err(&i2c->dev, "%s : fail to set ramp rate\n", __func__);
		return ret;
	}
...
static int rt5738_suspend(struct device *dev)
{
	int ret;
	struct rt5738_regulator_info *info = dev_get_drvdata(dev);

	pr_info("Entering suspend mode");

	ret = __rt5738_set_mode(info->regulator, REGULATOR_MODE_NORMAL);

	if (!ret)
		pr_info("Entered into Auto PWM mode and going to suspend");

	return ret;
}

static int rt5738_resume(struct device *dev)
{
	int ret;
	struct rt5738_regulator_info *info = dev_get_drvdata(dev);

	ret = __rt5738_set_mode(info->regulator, info->initial_mode);

	if (ret < 0)
		pr_err("Change regulator mode failed during resume");

	return ret;
}

static const struct dev_pm_ops rt5738_pm = {
	.suspend = rt5738_suspend,
	.resume = rt5738_resume,
};
