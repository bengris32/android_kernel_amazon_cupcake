...
int mtk_setbits_nonrst_reg2(u32 bit_mask, u32 value)
{
	...
	if (bit_mask & ~(THERMAL_SHUTDOWN_REASON_BIT_MASK |
...
/*
 * mtk_setbit_nonrst_reg2() - Sets a bit of WDT_NONRST_REG2 register.
 * @bit_pos - Position of the bit that needs to be set.
 *
 * Return: On success returns 0, on error returns -errno.
 */
static int mtk_setbit_nonrst_reg2(u8 bit_pos)
{
	struct mtk_wdt_dev *mtk_wdt;
	void __iomem *wdt_base;
	...
	mutex_lock(&mtk_wdt->wdt_nonrst_reg2_lock);
	iowrite32(ioread32(wdt_base + WDT_NONRST_REG2) | (1 << bit_pos),
			wdt_base + WDT_NONRST_REG2);
	mutex_unlock(&mtk_wdt->wdt_nonrst_reg2_lock);
	return 0;
}
				...
				void *cmd)
{
	...
	if (cmd && !strcmp(cmd, "rpmbpk")) {
		mtk_setbit_nonrst_reg2(BOOT_REASON_RPMBPK_BIT_POS);
	} else if (cmd && !strcmp(cmd, "recovery")) {
		mtk_setbit_nonrst_reg2(BOOT_REASON_RECOVERY_BIT_POS);
		#ifdef CONFIG_MT6397_MISC
		mtk_misc_mark_recovery();
		#endif
	} else if (cmd && !strcmp(cmd, "bootloader")) {
		mtk_setbit_nonrst_reg2(BOOT_REASON_BOOTLOADER_BIT_POS);
		#ifdef CONFIG_MT6397_MISC
		mtk_misc_mark_fast();
		#endif
	} else if (cmd && !strcmp(cmd, "kpoc")) {
		mtk_setbit_nonrst_reg2(BOOT_REASON_KPOC_BIT_POS);
		reg &= ~WDT_MODE_AUTO_START;
		#ifdef CONFIG_MT6397_MISC
		mtk_misc_mark_kpoc();
		#endif
	} else if (cmd && !strcmp(cmd, "enter_kpoc")) {
		mtk_setbit_nonrst_reg2(BOOT_REASON_ENTER_KPOC_BIT_POS);
		reg &= ~WDT_MODE_AUTO_START;
		#ifdef CONFIG_MT6397_MISC
		mtk_misc_mark_enter_kpoc();
		#endif
	} else if (cmd && !strcmp(cmd, "wmfk")) {
		mtk_setbit_nonrst_reg2(BOOT_REASON_WMFK_BIT_POS);
	}
