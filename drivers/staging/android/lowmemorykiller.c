...
/* Constants */
#define LMK_LOG_BUFFER_SIZE	(16 * 1024)
#define LMK_LOG_ELEMENT_SIZE	256
...
/* fosmod_fireos_crash_reporting begin */
	 ...
	 */
	if (lmk_log_buffer && selected &&
		(selected_oom_score_adj == 0 || selected_oom_score_adj == 1)) {
		foreground_kill = 1;
		head = lmk_log_buffer;
		buffer_remaining = LMK_LOG_BUFFER_SIZE;
		if (kill_msg_index && previous_crash)
			strncpy(previous_crash, kill_msg_index, LMK_LOG_ELEMENT_SIZE);
...
/* fosmod_fireos_crash_reporting begin */
	...
	lmk_log_buffer = kzalloc(LMK_LOG_BUFFER_SIZE, GFP_KERNEL);
	if (lmk_log_buffer) {
		buffer_end = lmk_log_buffer + LMK_LOG_BUFFER_SIZE;
		head = lmk_log_buffer;
		buffer_remaining = LMK_LOG_BUFFER_SIZE;
		foreground_kill = 0;
		kill_msg_index = NULL;
		previous_crash = kzalloc(LMK_LOG_ELEMENT_SIZE, GFP_KERNEL);
		if (!previous_crash)
			pr_info(KERN_ALERT "unable to allocate previous_crash for /proc/lmk_logs - previous_crash will not be logged");
	} else {
		pr_info(KERN_ALERT "unable to allocate buffer for /proc/lmk_logs - feature will be disabled");
	}
/* fosmod_fireos_crash_reporting end */
...
/* fosmod_fireos_crash_reporting begin */
#undef LMK_LOG_ELEMENT_SIZE
#undef LMK_LOG_BUFFER_SIZE
/* fosmod_fireos_crash_reporting end */
