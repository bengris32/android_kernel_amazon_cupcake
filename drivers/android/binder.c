...
static inline long copy_to_user_preempt_disabled(void __user *to,
...
static inline long copy_from_user_preempt_disabled(void *to,
	...
	if (tr->target.handle == 0) {
		for (i = 0; (2 * i) < tr->data_size; i++) {
			...
			*/
			...
			get_user_preempt_disabled(c, tmp);
