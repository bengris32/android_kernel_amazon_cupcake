...
static void snd_pcm_proc_xrun_count_read(struct snd_info_entry *entry,
					 struct snd_info_buffer *buffer)
{
	struct snd_pcm_str *pstr = entry->private_data;
	snd_iprintf(buffer, "%llu\n", pstr->xrun_count);
}

static void snd_pcm_substream_proc_xrun_count_read(struct snd_info_entry *entry,
					       struct snd_info_buffer *buffer)
{
	struct snd_pcm_substream *substream = entry->private_data;
	snd_iprintf(buffer, "%llu\n", substream->xrun_count);
}
...
static int snd_pcm_stream_proc_init(struct snd_pcm_str *pstr)
{
	...
	entry = snd_info_create_card_entry(pcm->card, "xrun_count",
						pstr->proc_root);
	if (entry != NULL) {
		snd_info_set_text_ops(entry, pstr,
				      snd_pcm_proc_xrun_count_read);
		if (snd_info_register(entry) < 0) {
			snd_info_free_entry(entry);
			entry = NULL;
		}
	}
	pstr->proc_xrun_count_entry = entry;
#endif
	return 0;
}
...
static int snd_pcm_stream_proc_done(struct snd_pcm_str *pstr)
{
	...
	snd_info_free_entry(pstr->proc_xrun_count_entry);
	pstr->proc_xrun_count_entry = NULL;
#endif
...
static int snd_pcm_substream_proc_init(struct snd_pcm_substream *substream)
{
	...
	entry = snd_info_create_card_entry(card, "xrun_count", substream->proc_root);
	if (entry != NULL) {
		snd_info_set_text_ops(entry, substream,
				      snd_pcm_substream_proc_xrun_count_read);
		if (snd_info_register(entry) < 0) {
			snd_info_free_entry(entry);
			entry = NULL;
		}
	}
	substream->proc_xrun_count_entry = entry;
...
static int snd_pcm_substream_proc_done(struct snd_pcm_substream *substream)
{
	...
	snd_info_free_entry(substream->proc_xrun_count_entry);
	substream->proc_xrun_count_entry = NULL;
#endif
