...
#define CDONE_MAX_READS 10

enum PINCTRL_PIN_STATE {
	PIN_STATE_DEFAULT = 0,
	PIN_STATE_PRE_DOWNLOAD,
	PIN_STATE_SPI_SS_HIGH,
	PIN_STATE_SPI_SS_LOW,
	PIN_STATE_SPI_SS_CS,
	PIN_STATE_MAX
};

static const char * const amzn_spi_pinctrl_pin_str[PIN_STATE_MAX] = {
	"default",
	"pre-download",
	"ss-high",
	"ss-low",
				...
				struct snd_pcm_hw_params *hw_params)
{
	...
	pr_info("%s: frame_bits=%u period_size=%lu periods=%u buffer_size=%lu rate=%d\n",
			__func__, runtime->frame_bits, runtime->period_size,
			runtime->periods, runtime->buffer_size, rate);
...
static void print_raw24(uint8_t *buf, size_t len)
{
	...
	if (len % DOUGH_AUDIO_FRAME_BYTES != 0) {
...
{
	int rc = 0, rst_gpio, cdone_gpio, i, cdone_value;
	...
	*/
	...
	spi_data.pinctrl = devm_pinctrl_get(&spi->dev);
	if (IS_ERR(spi_data.pinctrl)) {
		rc = PTR_ERR(spi_data.pinctrl);
		pr_warn("%s: pinctrl_get failed %d\n", __func__, rc);
		goto free_buffers;
	}

	/* Initialize SPI Pins */
	for (i = 0 ; i < PIN_STATE_MAX ; i++) {
		spi_data.pin_states[i] =
			pinctrl_lookup_state(spi_data.pinctrl,
				amzn_spi_pinctrl_pin_str[i]);
		if (IS_ERR(spi_data.pin_states[i])) {
			rc = PTR_ERR(spi_data.pin_states[i]);
			pr_warn("%s: Can't find pinctrl state %s %d\n",
				__func__, amzn_spi_pinctrl_pin_str[i], rc);
		}
	}

	if (!IS_ERR(spi_data.pin_states[PIN_STATE_DEFAULT])) {
		rc = pinctrl_select_state(spi_data.pinctrl,
				spi_data.pin_states[PIN_STATE_DEFAULT]);
		if (rc) {
			pr_err("%s: failed to select state %d\n", __func__, rc);
			goto free_buffers;
		}
	} else {
	    pr_err("%s: Unable to find PIN_STATE_DEFAULT\n", __func__);
	    goto free_buffers;
	}
	...
	/* Get CDONE Signal gpio */
	cdone_gpio = of_get_named_gpio(node, "fpga-cdone-gpio", 0);
	if (cdone_gpio < 0) {
		pr_err("%s: Error request FPGA CDONE GPIO %d\n", __func__,
		cdone_gpio);
		goto free_rgpio;
	}
	gpio_direction_input(cdone_gpio);

	/* SET CS to low. CSK to high */
	if (!IS_ERR(spi_data.pin_states[PIN_STATE_PRE_DOWNLOAD])) {
		rc = pinctrl_select_state(spi_data.pinctrl,
				spi_data.pin_states[PIN_STATE_PRE_DOWNLOAD]);
		if (rc) {
			pr_err("%s: failed to PIN_STATE_PRE_DOWNLOAD state %d\n",
			__func__, rc);
			goto free_cgpio;
		}
	} else {
		pr_err("%s: Unable to find PIN_STATE_PRE_DOWNLOAD\n", __func__);
		goto free_cgpio;
	}
	...
	/* SPI_SS High during dummy cycles */
	if (!IS_ERR(spi_data.pin_states[PIN_STATE_SPI_SS_HIGH])) {
		rc = pinctrl_select_state(spi_data.pinctrl,
			spi_data.pin_states[PIN_STATE_SPI_SS_HIGH]);
		if (rc) {
			pr_err("%s: failed to PIN_STATE_SPI_SS_HIGH state %d\n",
			__func__, rc);
			goto free_cgpio;
		}
	} else {
		pr_err("%s: Unable to find PIN_STATE_SPI_SS_HIGH\n", __func__);
		goto free_cgpio;
	}

	/* 8 dummy clk cycles */
	/* There isn't a way to generate 8 cycles from spi driver. This
	 * doesn't seem to cause issue eihter. Enable if needed
	rc = spi_txrx(spi, (void *)tx_df, (void *)rx_df,
			1, 0, 0);
	if (rc < 0) {
		pr_err("%s: Failed to send dummy cycles\n", __func__);
		goto free_cgpio;
	}
	*/

	/* Check CDONE. Should be low */
	cdone_value = gpio_get_value(cdone_gpio);
	pr_info("%s: Before Download. FPGA CDONE = %d\n",
		__func__, cdone_value);

	/* SPI_SS low on downloading */
	if (!IS_ERR(spi_data.pin_states[PIN_STATE_SPI_SS_LOW])) {
		rc = pinctrl_select_state(spi_data.pinctrl,
				spi_data.pin_states[PIN_STATE_SPI_SS_LOW]);
		if (rc) {
			pr_err("%s: failed to PIN_STATE_SPI_SS_LOW state %d\n",
			__func__, rc);
			goto free_cgpio;
		}
	} else {
		pr_err("%s: Unable to find PIN_STATE_SPI_SS_LOW\n", __func__);
		goto free_cgpio;
	}
	...
	if (rc < 0) {
		pr_err("%s: FPGA load failed to initiate sync SPI transfer error %d\n",
			__func__, rc);
		release_firmware(fw_entry);
		goto free_cgpio;
	}
	...
	while (i < CDONE_MAX_READS) {
		cdone_value = gpio_get_value(cdone_gpio);
		pr_info("%s: CDONE FPGA = %d\n", __func__, cdone_value);
		if (cdone_value)
			break;
		msleep(1);
		i++;
	...
	/* SPI_SS CS */
	if (!IS_ERR(spi_data.pin_states[PIN_STATE_SPI_SS_CS])) {
		rc = pinctrl_select_state(spi_data.pinctrl,
				spi_data.pin_states[PIN_STATE_SPI_SS_CS]);
		if (rc) {
			pr_err("%s: failed to PIN_STATE_SPI_SS_CS state %d\n",
			__func__, rc);
			goto free_cgpio;
		}
	} else {
		pr_err("%s: Unable to find PIN_STATE_SPI_SS_CS\n", __func__);
		goto free_cgpio;
	}
