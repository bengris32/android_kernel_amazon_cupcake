	...
	int bit_width)
{
	...
	bit_width *= (1 << afe->bus_bclk_factors[MT8167_AFE_BUS_I2S1]);
	if (bit_width > 16)
	...
	int bit_width)
{
	...
	bit_width *= (1 << afe->bus_bclk_factors[MT8167_AFE_BUS_I2S3]);
	if (bit_width > 16)
	...
	int bit_width)
{
	...
	if (afe->bus_l_r_swap[MT8167_AFE_BUS_I2S2])
		val |= AFE_I2S_CON2_L_R_SWAP;

	bit_width *= (1 << afe->bus_bclk_factors[MT8167_AFE_BUS_I2S2]);
	if (bit_width > 16)
		val |= AFE_I2S_CON2_WLEN_32BIT;

	/* bit24~28 is important for latching data from iis.
	 * default value(0x8) should be kept generally.
	 */
	regmap_update_bits(afe->regmap, AFE_I2S_CON2,
		~(u32)(AFE_I2S_CON2_EN | AFE_I2S_CON2_UPDATE_WORD), val);
	...
	int bit_width)
{
	...
	bit_width *= (1 << afe->bus_bclk_factors[MT8167_AFE_BUS_I2S0]);
	if (bit_width > 16)
...
static int mt8167_afe_set_mrg(struct mtk_afe *afe, unsigned int rate)
{
	...
	val |= AFE_DAIBT_CON0_USE_MRG_INPUT | AFE_DAIBT_CON0_DATA_DRY;
...
static int mt8167_afe_enable_irq(struct mtk_afe *afe, struct mt8167_afe_memif *memif)
{
	...
	case MT8167_AFE_IRQ_11:
		regmap_update_bits(afe->regmap, AFE_TSF_CON,
			...
			AFE_TSF_CON_DL2_AUTO_EN | AFE_TSF_CON_DL1_AUTO_EN |
			AFE_TSF_CON_DL_HW_DIS,
			AFE_TSF_CON_HDMIOUT_HW_DIS | AFE_TSF_CON_DL_HW_DIS);
		regmap_update_bits(afe->regmap, AFE_IRQ_MCU_CON2,
							1 << 5, 1 << 5);
		break;
	case MT8167_AFE_IRQ_13:
		regmap_update_bits(afe->regmap, AFE_IRQ_MCU_CON2, 1 << 7, 1 << 7);
		break;
	case MT8167_AFE_IRQ_ACC2:
		regmap_update_bits(afe->regmap, AFE_TSF_CON,
			AFE_TSF_CON_HDMIOUT_AUTO_EN |
			AFE_TSF_CON_HDMIOUT_HW_DIS |
			AFE_TSF_CON_DL2_AUTO_EN | AFE_TSF_CON_DL1_AUTO_EN |
...
static int mt8167_afe_disable_irq(struct mtk_afe *afe, struct mt8167_afe_memif *memif)
{
	...
	case MT8167_AFE_IRQ_ACC2:
		regmap_update_bits(afe->regmap, AFE_IRQ_MCU_CON2,
					1 << 1, 0 << 1);
		regmap_write(afe->regmap, AFE_IRQ_CLR, 1 << 14);
		break;
	default:
		break;
	}
						...
						struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK ||
		...
		dev_info(afe->dev, "-%s() Playback_Audio\n", __func__);
	}
				 ...
				 struct snd_soc_dai *dai)
{
	...
	if (be->prepared[stream]) {
		...
		if (reset_i2s_in_change) {
			if (afe->bus_blrclk_modes[MT8167_AFE_BUS_I2S2])
						 ...
						 struct snd_soc_dai *dai)
{
	...
	if (apply_i2s_out_change) {
		...
		if (afe->i2s1and2_mclk_double_rate)
			mt8167_afe_dais_set_clks(afe, afe->clocks[MT8167_CLK_APLL12_DIV1],
				rate * MT8167_I2S1_MCLK_MULTIPLIER * 2, NULL, 0);
		else
			mt8167_afe_dais_set_clks(afe, afe->clocks[MT8167_CLK_APLL12_DIV1],
	...
	if (apply_i2s_in_change) {
		...
		if (afe->i2s1and2_mclk_double_rate)
			mt8167_afe_dais_set_clks(afe, afe->clocks[MT8167_CLK_APLL12_DIV2],
				rate * MT8167_I2S2_MCLK_MULTIPLIER * 2, NULL, 0);
		else
			mt8167_afe_dais_set_clks(afe, afe->clocks[MT8167_CLK_APLL12_DIV2],
				...
				struct snd_soc_dai *dai)
{
	...
	if (be->prepared[stream]) {
		/* keep or disable tdm */
		if (afe->bus_blrclk_modes[MT8167_AFE_BUS_TDM_OUT])
			regmap_update_bits(afe->regmap, AFE_TDM_CON1, 0x1, 1);
		else
				...
				struct snd_soc_dai *dai)
{
	...
	if ((tdm_out_mode == MT8167_AFE_TDM_OUT_I2S ||
		...
		val |= AFE_TDM_CON1_1_BCK_DELAY | AFE_TDM_CON1_LRCK_INV;
				...
				struct snd_soc_dai *dai)
{
	...
	case SND_SOC_DAIFMT_IB_IF:
		val |= AFE_TDM_IN_CON1_LRCK_INV | AFE_TDM_IN_CON1_BCK_INV;
		break;
	...
	if (afe->tdm_in_lrck_cycle == LRCK_CYCLE_INVALID) {
		val2 = phy_width > 16 ? 32 : 16;
		...
		*/
		...
		if (memif->data->irq_cnt_shift >= 0)
			regmap_update_bits(afe->regmap,
				...
				memif->data->irq_cnt_mask << memif->data->irq_cnt_shift,
				...
				struct snd_soc_dai *dai)
{
	...
	regmap_update_bits(afe->regmap,
		AFE_GAIN1_CON0, AFE_GAIN1_CON0_MODE_MASK, (unsigned int)fs<<4);
	...
	if ((val1 & AFE_GAIN1_CON1_MASK) != (val2 & AFE_GAIN1_CUR_MASK))
		regmap_update_bits(afe->regmap,
			AFE_GAIN1_CUR, AFE_GAIN1_CUR_MASK, val1);

	regmap_update_bits(afe->regmap,
		AFE_GAIN1_CON0, AFE_GAIN1_CON0_EN_MASK, 1);
	be->prepared[stream] = true;
			...
			unsigned int mode)
{
	...
	case MT8167_AFE_IRQ_11:
		...
		break;
	case MT8167_AFE_IRQ_ACC2:
		data->irq_reg_cnt = AFE_IRQ_CNT_ACC2;
		data->irq_cnt_shift = 0;
		data->irq_cnt_mask = 0xffffffff,
		data->irq_mode = MT8167_AFE_IRQ_ACC2;
		data->irq_fs_reg = AFE_IRQ_MCU_CON2;
		data->irq_fs_shift = 12;
		data->irq_clr_shift = 14;
		break;
	default:
		ret = -EINVAL;
	}
...
static const struct snd_kcontrol_new mt8167_afe_o13_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("I00 Switch", AFE_GAIN1_CONN2, 2, 1, 0),
};

static const struct snd_kcontrol_new mt8167_afe_o14_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I04 Switch", AFE_GAIN1_CONN2, 19, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I01 Switch", AFE_GAIN1_CONN2, 16, 1, 0),
};

static const struct snd_kcontrol_new mt8167_afe_o11_mix[] = {
	SOC_DAPM_SINGLE_AUTODISABLE("I02 Switch", AFE_CONN3, 6, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I10 Switch", AFE_GAIN1_CONN, 30, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("I11 Switch", AFE_GAIN1_CONN, 31, 1, 0),
};
...
static void mt8167_afe_preset_clks(struct mtk_afe *afe)
{
	unsigned int val;
	unsigned int def_mclk = 12288000;
	unsigned int def_lrclk = 48000;

	if (afe->i2s1and2_mclk_double_rate)
	    def_mclk *= 2;

	if (afe->bus_mclk_modes[MT8167_AFE_BUS_NUM]) {
		mt8167_afe_enable_main_clk(afe);
		mt8167_afe_enable_apll_associated_cfg(afe, MT8167_AFE_APLL2);

		/* enable afe, if not, there is no bclk/lrclk on the boot;
		 */
		mt8167_afe_enable_afe_on(afe);
	}

	/* i2s1 mclk/blrclk */
	if (afe->bus_mclk_modes[MT8167_AFE_BUS_I2S1]) {
		clk_set_parent(afe->clocks[MT8167_CLK_I2S1_M_SEL],
					afe->clocks[MT8167_CLK_AUD2]);
		mt8167_afe_dais_set_clks(afe,
			afe->clocks[MT8167_CLK_APLL12_DIV1], def_mclk, NULL, 0);
		mt8167_afe_dais_enable_clks(afe,
					afe->clocks[MT8167_CLK_APLL12_DIV1],
					NULL);
	}
	if (afe->bus_blrclk_modes[MT8167_AFE_BUS_I2S1]) {
		mt8167_afe_set_i2s_out(afe, def_lrclk, 16);
		mt8167_afe_set_i2s_out_enable(afe, true);
	}

	/* i2s2 mclk/blrclk */
	if (afe->bus_mclk_modes[MT8167_AFE_BUS_I2S2]) {
		clk_set_parent(afe->clocks[MT8167_CLK_I2S2_M_SEL],
					afe->clocks[MT8167_CLK_AUD2]);
		mt8167_afe_dais_set_clks(afe,
			afe->clocks[MT8167_CLK_APLL12_DIV2], def_mclk, NULL, 0);
		mt8167_afe_dais_enable_clks(afe,
					afe->clocks[MT8167_CLK_APLL12_DIV2],
					NULL);
	}
	if (afe->bus_blrclk_modes[MT8167_AFE_BUS_I2S2]) {
		mt8167_afe_set_i2s_in(afe, def_lrclk, 16);
		mt8167_afe_set_i2s_in_enable(afe, true);
	}

	/* tdm out mclk/blrclk */
	if (afe->bus_mclk_modes[MT8167_AFE_BUS_TDM_OUT]) {
		clk_set_parent(afe->clocks[MT8167_CLK_I2S4_M_SEL],
					afe->clocks[MT8167_CLK_AUD2]);
		mt8167_afe_dais_set_clks(afe,
			afe->clocks[MT8167_CLK_APLL12_DIV4], def_mclk, NULL, 0);
		mt8167_afe_dais_enable_clks(afe,
					afe->clocks[MT8167_CLK_APLL12_DIV4],
					NULL);
	}
	if (afe->bus_blrclk_modes[MT8167_AFE_BUS_TDM_OUT]) {
		mt8167_afe_dais_set_clks(afe,
			...
			afe->clocks[MT8167_CLK_APLL12_DIV4B],
			def_lrclk * 2 * 16);
		mt8167_afe_dais_enable_clks(afe, NULL,
					afe->clocks[MT8167_CLK_APLL12_DIV4B]);
		val = AFE_TDM_CON1_MSB_ALIGNED;
		val |= AFE_TDM_CON1_1_BCK_DELAY | AFE_TDM_CON1_LRCK_INV;
		val |= AFE_TDM_CON1_WLEN_16BIT |
		...
		regmap_update_bits(afe->regmap, AUDIO_TOP_CON3,
				AUD_TCON3_HDMI_BCK_INV, AUD_TCON3_HDMI_BCK_INV);
		regmap_update_bits(afe->regmap, AFE_I2S_CON1,
...
static void mt8167_afe_unset_clks(struct mtk_afe *afe)
{

	if (afe->bus_blrclk_modes[MT8167_AFE_BUS_TDM_OUT]) {
		mt8167_afe_disable_top_cg(afe, MT8167_AFE_CG_HDMI);
		mt8167_afe_dais_disable_clks(afe, NULL,
					afe->clocks[MT8167_CLK_APLL12_DIV4B]);
	}
	if (afe->bus_mclk_modes[MT8167_AFE_BUS_TDM_OUT])
...
static int mt8167_afe_pcm_dev_probe(struct platform_device *pdev)
{
	...
	spin_lock_init(&afe->spdifin_ctrl_lock);
	...
	if (of_property_read_u32(np, "mediatek,i2s-1-2-mclk-in-double-rate",
				&afe->i2s1and2_mclk_double_rate))
		afe->i2s1and2_mclk_double_rate = 0;
	...
	for (i = 0; i < MT8167_AFE_I2S_SETS; i++) {
		...
		dev_info(&pdev->dev, "MTK AFE driver i=%d i2s_clk_modes=%d\n", i, afe->i2s_clk_modes[i]);
	}

	/* the flag about mclk and blrclk always-on is derived from
	 * dts file. read these value for 6 buses to determin which
	 * clocks should be kept on always:
	 * I2S0, I2S1, I2S2, I2S3, TDM out(8ch-I2S out) and TDM in.
	 */
	of_property_read_u32_array(np, "mediatek,mclk-always-on",
		afe->bus_mclk_modes, ARRAY_SIZE(afe->bus_mclk_modes) - 1);

	of_property_read_u32_array(np, "mediatek,blrclk-always-on",
		afe->bus_blrclk_modes, ARRAY_SIZE(afe->bus_blrclk_modes));

	/* enable mclk needs two steps. one is common and others are based
	 * on which mclk is planned to be enabled. it's not necessary for
	 * all bus mclks requested to do common step many times. Only one
	 * time is need even more than one mclks are need. a value about
	 * request times is calculated for this purpose.
	 */
	i = MT8167_AFE_BUS_I2S0;
	for (; i < MT8167_AFE_BUS_NUM; i++)
		afe->bus_mclk_modes[MT8167_AFE_BUS_NUM]
			+= afe->bus_mclk_modes[i];

	/* two factor for bclk:
	 *   - 32fs if bit-depth = 16 with 2 channel, and
	 *     64fs if bit-depth = 32/24.
	 *   - always 64fs no matter what the bit-depth is
	 * 0 means variable bclk factor, and 1 means in-variable
	 * bclk factor(64fs for 2 channel audio data)
	 */
	of_property_read_u32_array(np, "mediatek,bclk-factors",
		afe->bus_bclk_factors, ARRAY_SIZE(afe->bus_bclk_factors));
	...
	if (of_property_read_u32(np, "mediatek,dl2-irq-mode",
				&afe->dl2_irq_mode))
	...
	if (afe->dl2_irq_mode != memif_data[MT8167_AFE_MEMIF_DL2].irq_mode)
		mt8167_afe_set_memif_irq_by_mode(
			&memif_data[MT8167_AFE_MEMIF_DL2],
			afe->dl2_irq_mode);
	...
	if (of_property_read_u32_array(np, "mediatek,l_r_swap",
		afe->bus_l_r_swap, ARRAY_SIZE(afe->bus_l_r_swap)))
		dev_info(&pdev->dev, "mediatek,l_r_swap is not defined in dts.");
	...
	dev_info(&pdev->dev, "MTK AFE driver initialized. tdm_out_mode=%d double=%d\n",
	    afe->tdm_out_mode, afe->i2s1and2_mclk_double_rate);
	return 0;
